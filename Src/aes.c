...
static void XorWith(uint8_t* buf, const uint8_t* Iv)
{
  uint8_t i;
...
{
  uint8_t tbuf[AES_BLOCKLEN];
  uint32_t i;
  for (i=0; i<length; i+=AES_BLOCKLEN)
  {
    memcpy(tbuf, ctx->Iv, AES_BLOCKLEN);
    Cipher((state_t*)tbuf, ctx->RoundKey);
    XorWith(buf, tbuf);
    GenNewIV(ctx->Iv);
  }
}
